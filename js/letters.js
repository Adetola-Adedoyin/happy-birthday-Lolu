...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function createEnvelopes(data) {
    data.letters.forEach(letter => {
      const envelope = document.createElement("div");
      envelope.className = "envelope sparkle";
      envelope.innerHTML = `
      ...
      envelope.addEventListener("click", () => {
        ...
        letterDiv.classList.toggle("hidden");
  ...
  // Try to fetch JSON, fallback to embedded data
  fetch("data/letters.json")
    .then(res => res.json())
    .then(data => createEnvelopes(data))
    .catch(() => createEnvelopes(fallbackData));
});
